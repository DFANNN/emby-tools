...
router.get('/todayTrending', async (req, res) => {
  console.log(`${_now()} [TMDB TRENDING] 开始获取今日趋势`)
  try {
    ...
    console.log(`${_now()} [TMDB TRENDING] 获取今日趋势成功 - 共 ${trendList.length} 项`)
    return res.success(trendList)
  } catch (error) {
    console.error(`${_now()} [TMDB TRENDING] 获取今日趋势失败:`, error.message)
...
router.get('/weekTrending', async (req, res) => {
  console.log(`${_now()} [TMDB TRENDING] 开始获取本周趋势`)
  try {
    const baseUrl = 'https://image.tmdb.org/t/p/original'
    ...
    console.log(`${_now()} [TMDB TRENDING] 获取本周趋势成功 - 共 ${trendList.length} 项`)
    return res.success(trendList)
  } catch (error) {
    console.error(`${_now()} [TMDB TRENDING] 获取本周趋势失败:`, error.message)
    if (isTimeoutError(error)) {
      return res.error('请求超时无法访问 TMDB，请自行配置代理或 DNS/hosts 后重试。')
    }
    return res.error(error)
...
router.get('/movieNowPlaying', async (req, res) => {
  console.log(`${_now()} [TMDB MOVIE] 开始获取正在上映的电影`)
  try {
    const baseUrl = 'https://image.tmdb.org/t/p/original'
    ...
    console.log(`${_now()} [TMDB MOVIE] 获取正在上映电影成功 - 共 ${nowPlayingList.length} 项`)
    return res.success(nowPlayingList)
  } catch (error) {
    console.error(`${_now()} [TMDB MOVIE] 获取正在上映电影失败:`, error.message)
    if (isTimeoutError(error)) {
      return res.error('请求超时无法访问 TMDB，请自行配置代理或 DNS/hosts 后重试。')
    }
    return res.error(error)
...
router.get('/moviePopular', async (req, res) => {
  ...
  console.log(`${_now()} [TMDB MOVIE] 开始获取热门电影 - 页码: ${page || 1}`)
  try {
    const baseUrl = 'https://image.tmdb.org/t/p/original'
    ...
    console.log(`${_now()} [TMDB MOVIE] 获取热门电影成功 - 共 ${list.length} 项`)
    return res.success(list)
  } catch (error) {
    console.error(`${_now()} [TMDB MOVIE] 获取热门电影失败:`, error.message)
    if (isTimeoutError(error)) {
      return res.error('请求超时无法访问 TMDB，请自行配置代理或 DNS/hosts 后重试。')
    }
    return res.error(error)
...
router.get('/tvPopular', async (req, res) => {
  const { page } = req.query
  console.log(`${_now()} [TMDB TV] 开始获取热门剧集 - 页码: ${page || 1}`)
  try {
    const baseUrl = 'https://image.tmdb.org/t/p/original'
    ...
    console.log(`${_now()} [TMDB TV] 获取热门剧集成功 - 共 ${list.length} 项`)
    return res.success(list)
  } catch (error) {
    console.error(`${_now()} [TMDB TV] 获取热门剧集失败:`, error.message)
    if (isTimeoutError(error)) {
      return res.error('请求超时无法访问 TMDB，请自行配置代理或 DNS/hosts 后重试。')
    }
    return res.error(error)
...
router.get('/movieTopRated', async (req, res) => {
  const { page } = req.query
  console.log(`${_now()} [TMDB MOVIE] 开始获取高分电影 - 页码: ${page || 1}`)
  try {
    const baseUrl = 'https://image.tmdb.org/t/p/original'
    ...
    console.log(`${_now()} [TMDB MOVIE] 获取高分电影成功 - 共 ${list.length} 项`)
    return res.success(list)
  } catch (error) {
    console.error(`${_now()} [TMDB MOVIE] 获取高分电影失败:`, error.message)
    if (isTimeoutError(error)) {
      return res.error('请求超时无法访问 TMDB，请自行配置代理或 DNS/hosts 后重试。')
    }
    return res.error(error)
...
router.get('/tvTopRated', async (req, res) => {
  const { page } = req.query
  console.log(`${_now()} [TMDB TV] 开始获取高分剧集 - 页码: ${page || 1}`)
  try {
    const baseUrl = 'https://image.tmdb.org/t/p/original'
    ...
    console.log(`${_now()} [TMDB TV] 获取高分剧集成功 - 共 ${list.length} 项`)
    return res.success(list)
  } catch (error) {
    console.error(`${_now()} [TMDB TV] 获取高分剧集失败:`, error.message)
    if (isTimeoutError(error)) {
      return res.error('请求超时无法访问 TMDB，请自行配置代理或 DNS/hosts 后重试。')
    }
    return res.error(error)
...
router.get('/discover', async (req, res) => {
  ...
  console.log(
    `${_now()} [TMDB DISCOVER] 开始题材发现 - 类型: ${media_type}, 页码: ${
      pageParam || 'random'
    }, 类型: ${with_genres}, 数量: ${randomNum || 9}`
  )
  try {
    const baseUrl = 'https://image.tmdb.org/t/p/original'
    ...
    console.log(`${_now()} [TMDB DISCOVER] 题材发现成功 - 返回 ${picked.length} 张图片`)
    return res.success(picked)
  } catch (error) {
    console.error(`${_now()} [TMDB DISCOVER] 题材发现失败:`, error.message)
    if (isTimeoutError(error)) {
      return res.error('请求超时无法访问 TMDB，请自行配置代理或 DNS/hosts 后重试。')
    }
    return res.error(error)
